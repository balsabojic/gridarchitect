 ...
 * @author balsa
 */
public class ConsumptionProfleFortiss {
	...
	public static double getLoadFortissConsumption(LocalDateTime inputTime){
	...
	public static boolean readFile(){
		try {
			String source = "src/main/resources/lastProfiles/fortiss-consumption.csv";
			
			String location = ConsumptionProfleFortiss.class.getProtectionDomain().getCodeSource().getLocation().getPath();
			...
			e.printStackTrace();
			try {
				String source = "/resources/lastProfiles/fortiss-consumption.csv";
				InputStreamReader isr = new InputStreamReader(ConsumptionProfleFortiss.class.getResourceAsStream(source));					
	...
	private static void read(BufferedReader br) throws IOException, ParseException{
    	...
    	while ((zeile = br.readLine()) != null){		    			
				...
				String result = buffer[1].toString();
				
				loadDomestic[i]= Double.parseDouble(result);							
				i++;
		}
